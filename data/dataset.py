...
import qwen3_vl.data.datasets_mixture as datasets_mixture
from qwen3_vl import conversation as conversation_lib
from qwen3_vl.constants import (
...
from qwen3_vl.eval.mmmu_utils.data_utils import CAT_SHORT2LONG, construct_prompt, load_yaml, process_single_sample
from qwen3_vl.mm_utils import opencv_extract_frames, process_image, tokenizer_image_token
from qwen3_vl.model import *
from qwen3_vl.train.args import DataArguments, TrainingArguments
from qwen3_vl.train.sequence_parallel import (
    extract_local_from_list,
    extract_local_input_ids,
    extract_local_position_ids,
    get_pg_manager,
)
from qwen3_vl.utils.logging import logger
from qwen3_vl.utils.tokenizer import preprocess_conversation
...
class LazySupervisedDataset(Dataset):
    ...
    def _load_video(video_path, num_video_frames, loader_fps, data_args, fps=None, frame_count=None):
        ...
        from qwen3_vl.mm_utils import opencv_extract_frames
...
class LazyMMC4Dataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        # image-token labels are kept for the current Qwen3-VL training format
...
class LazyCoyoDataset_LONGSEQ(Dataset):
    ...
    def __init__(
        ...
        from qwen3_vl.train.qwen3_vl_dpsp_attn_monkey_patch import get_sequence_parallel_rank, get_sequence_parallel_size
...
class LazyCCSWebDataset(Dataset):
    ...
    def __init__(
        ...
        from qwen3_vl.data.simple_nav_webdataset import NavWebDataset

        print("[DEBUG] ", osp.abspath(data_path))
        self.dataset = NavWebDataset(data_path=osp.abspath(data_path))
...
class LazyEvaluateDataset(LazySupervisedDataset):
    def __init__(
        ...
        config_path: str = "qwen3_vl/eval/mmmu_utils/configs/qwen3_vl.yaml",
        split="validation",
        **kwargs,
    ):
...
class LazyCoyoWebDataset(Dataset):
    ...
    def __init__(
        ...
        print("[DEBUG] ", osp.abspath(data_path))
        self.dataset = NavWebDataset(data_path=osp.abspath(data_path), meta_path=data_args.meta_path)
...
class NavLongSeq(Dataset):
    ...
    def __init__(
        ...
        raise ValueError(
            "This class NavLongSeq is deprecated. You can use naive dataset directly for supporting seq parallel"
        )

        from qwen3_vl.data.simple_nav_webdataset import NavWebDataset

        data_path = osp.expanduser(data_path)
        self.dataset = NavWebDataset(data_path=data_path)
        self.data_path = data_path
        self.tokenizer = tokenizer
        self.data_args = data_args
...
class LazyEnvDropDataset(Dataset):
    ...
    def _load_video(video_path, num_video_frames, data_args):
        import cv2
        from qwen3_vl.mm_utils import get_frame_from_vcap_vlnce
...
class LazyVLNCEDataset(Dataset):
    ...
    def _load_video(video_paths, num_video_frames, data_args):
        from qwen3_vl.mm_utils import vlnce_frame_sampling
