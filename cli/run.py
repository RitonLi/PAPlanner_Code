...
def main() -> None:
    ...
    if "NAV_SLURM_ACCOUNT" not in os.environ or "NAV_SLURM_PARTITION" not in os.environ:
        raise ValueError("`NAV_SLURM_ACCOUNT` and `NAV_SLURM_PARTITION` must be set in the environment.")
    account = os.environ["NAV_SLURM_ACCOUNT"]
    partition = os.environ["NAV_SLURM_PARTITION"]
