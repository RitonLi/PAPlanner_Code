...
from qwen3_vl.eval import EVAL_ROOT, TASKS
...
def main() -> None:
    ...
    parser.add_argument("--conv-mode", "-c", type=str, default="qwen_vl")
    ...
    for task in tasks:
        ...
        cmd = []
        if task.startswith("qw-"):
            cmd += [f"{EVAL_ROOT}/qw.sh", task.replace("qw-", ""), args.model_path]
        elif "-" in task:
        ...
        if os.environ.get("SLURM_JOB_ID"):
            ...
            concurrency = 10
            cmd = [f"nav-run -m eval -J {model_name}/{task}"] + cmd
