...
from qwen3_vl.constants import DEFAULT_IM_END_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IMAGE_TOKEN, IMAGE_TOKEN_INDEX
from qwen3_vl.conversation import SeparatorStyle, conv_templates
from qwen3_vl.mm_utils import (
...
from qwen3_vl.utils import disable_torch_init
...
def postprocess_2d_grounding(output, h, w, box_format="qwen3_vl"):
    h = float(h)
    w = float(w)
    output = output.strip()
    if output.endswith("."):
    ...
    if "qwen3_vl" in box_format:
        # NOTE: box format is on image coordinate AFTER square pad.
...
def eval_model(args):
    ...
    stopping_criteria = (
        ...
        if args.conv_mode == "v0"
        else None
    )

    box_format = getattr(model.config, "box_format", "qwen3_vl") if args.box_format is None else args.box_format
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-path", type=str, default="/PATH_TO_YOUR_CHECKPOINT")
    ...
    parser.add_argument("--project", type=str, default="Nav-RefCOCO-Eval")
